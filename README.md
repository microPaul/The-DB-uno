...
So, the question becomes, if a typical UNO clone was modified by removing the ATmega328p (32 pin package) and replaced with an AVR128DB32 (32 pin package) to become a DBuno, what would be the assignments from MCU ports to the signal connectors at the edge of the DBuno board?

I’ve given a fair amount of thought to have and have a spread sheet showing how I think that should be done.  In doing so, I considered the following assumptions and goals:

1.	It’s more difficult to change a printed circuit board than it is firmware, so the firmware should accommodate the hardware.
...
5.	Ensure that all available PCx pins are provided for use since they are powered from VDDIO2 and allow for different port voltage than that used for system voltage.
...
USART-0 (ALT) (PA5, PA4) appears on DBuno edge pins D0 and D1, placing a USART for the DBuno on the same pins as the original UNO.

SPI-1 (PC3, PC0, PC1, PC2) appears on DBuno edge pins D10, D11, D12 and D13, placing an SPI port for the DBuno on the same pins as the original UNO.

I2C-0 (PA2, PA3) appears on the DBuno edge pins SDA and SCL.


Additional pin port functions are as follows:
...
I2C-1 (PF2, PF3) appears on DBuno edge pins A4 and A5, placing an I2C port for the DBuno on the same pins as the original UNO (note that on the original UNO the SDA and SCL pins were added as a later REV of the board, but they were cross-connected to A4 and A5 on the original UNO.  On the DBuno pins SDA/SCL are independent of pins A4/A5 and use two independent I2C ports).
...
If the SPI-1 port at D10-D13 is not used by firmware, the firmware can assign these pints to USART-1 (TX, RX, DIR, CK).
...
In the case of the 28 pin DIP part (thank you Microchip for including a DIP package in the mix for those who have difficulty soldering surface mount parts), we loose PF2, PF3 (access to I2C-1) and PF4 and PF5 (access to USART-2), which leaves open DBuno edge pins A4, A5, D8 and D9.

So for a ARV-DB28 pin assignment, the external HF oscillators pins PA0 and P1 will be brought out to D8 and D9 respectively through solder bridge jumpers (closed if crystal is not populated, open if crystal is populated) and PD5 that was abandoned in the 32 pin configuration will be connected to DBuno edge pin A4.  DBuno edge pins A5 will be left open circuit.
...
These assignments can be seen as a whole on a spread sheet at this link.
